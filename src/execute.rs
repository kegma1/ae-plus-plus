        ...
        match token.op {
            ...
            ops::Operator::Add => {
                ...
                match (a, b) {
                    ...
                    (ops::Value::Float(x), ops::Value::Float(y)) => ctx.push(ops::Value::Float(x + y)),
            ...
            ops::Operator::Div => {
                ...
                match (a, b) {
                    (ops::Value::Int(x), ops::Value::Int(y)) => {ctx.push(ops::Value::Int(x % y)); ctx.push(ops::Value::Int(x / y))},
                    (ops::Value::Float(x), ops::Value::Float(y)) => {ctx.push(ops::Value::Float(x % y)); ctx.push(ops::Value::Float(x / y))},
                    (_, _) => {
                        let err_s: String = format!("'{} / {}' er ikke støttet", a, b).to_owned();
                        return Err((Box::leak(err_s.into_boxed_str()), token.pos.clone()));
                    }
            ...
            ops::Operator::Eq => {
                ...
                match (a, b) {
                    ...
                    (ops::Value::Float(x), ops::Value::Float(y)) => ctx.push(ops::Value::Bool(x == y)),
                    (ops::Value::Str(x), ops::Value::Str(y)) => ctx.push(ops::Value::Bool(ctx.str_heap[x] == ctx.str_heap[y])),
            ...
            ops::Operator::Cast => {
                if ctx.stack.len() < 2 {
                    return Err(("'omgjør' operator krever minst 1 argument", token.pos.clone()));
                }
                ...
                match (typ, b) {
                    (ops::Value::TypeLiteral(ops::TypeLiteral::Int), _) => {
                        match b {
                            ...
                            ops::Value::Bool(x) => if x { ctx.push(ops::Value::Int(1i32))} else {ctx.push(ops::Value::Int(0i32))},
                            ops::Value::Str(x) => {
                    ...
                    (ops::Value::TypeLiteral(ops::TypeLiteral::Float), _) => {
                        match b {
                            ...
                            ops::Value::Bool(x) => if x { ctx.push(ops::Value::Float(1.))} else {ctx.push(ops::Value::Float(0.))},
                            ops::Value::Str(x) => {
